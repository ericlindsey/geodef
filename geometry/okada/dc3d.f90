      ...
      SUBROUTINE  DC3D(ALPHA,X,Y,Z,DEPTH,DIP,AL1,AL2,AW1,AW2,DISL1,DISL2,DISL3,UX,UY,UZ,UXX,UYX,UZX,UXY,UYY,UZY,UXZ,UYZ,UZZ,IRET)
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*4   ALPHA,X,Y,Z,DEPTH,DIP,AL1,AL2,AW1,AW2,DISL1,DISL2,DISL3,UX,UY,UZ,UXX,UYX,UZX,UXY,UYY,UZY,UXZ,UYZ,UZZ
!
!********************************************************************
!*****                                                          *****
...
!--------------------------------
!----- ON FAULT EDGE
      IF(Q.EQ.F0 .AND.(    (XI(1)*XI(2).LE.F0 .AND. ET(1)*ET(2).EQ.F0).OR.(ET(1)*ET(2).LE.F0 .AND. XI(1)*XI(2).EQ.F0) )) THEN
        IRET=1
        GO TO 99
      ENDIF
...
!
      ...
      COMMON /C2/XI2,ET2,Q2,R,R2,R3,R5,Y,D,TT,ALX,ALE,X11,Y11,X32,Y32,EY,EZ,FY,FZ,GY,GZ,HY,HZ
...
!-----
      ...
      IF(CD.NE.F0) THEN
        ...
        ELSE
          X=DSQRT(XI2+Q2)
          AI4=F1/CDCD*( XI/RD*SDCD+F2*DATAN((ET*(X+Q*CD)+X*(R+X)*SD)/(XI*(R+X)*CD)) )
        ENDIF
...
!
      COMMON /C0/DUMMY(5),SD,CD
      COMMON /C1/P,Q,S,T,XY,X2,Y2,D2,R,R2,R3,R5,QR,QRX,A3,A5,B3,C3,UY,VY,WY,UZ,VZ,WZ
...
! Variable declarations
      REAL :: ALPHA, X, Y, Z, DEPTH, DIP, AL1, AL2, AW1, AW2
      REAL :: DISL1, DISL2, DISL3
      REAL :: UX, UY, UZ
      REAL :: UXX, UYX, UZX, UXY, UYY, UZY, UXZ, UYZ, UZZ
      INTEGER :: IRET
