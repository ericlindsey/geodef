...
def geod2ecef(
    ...
    if crs is not None:
        ...
        if importlib.util.find_spec("pyproj") is None:
            raise ImportError(
                "The 'pyproj' library is required to use the 'crs' "
                "parameter. Install it with 'pip install pyproj'."
            )
        import pyproj
    ...
    z = (curvn * (1.0 - ellps.e2) + np.array(alt)) * sinlat
    return x, y, z
...
def ecef2geod(
    ...
    """
    Convert ECEF coordinates (X,Y,Z) in meters to geodetic (lat,lon,alt)
    in degrees/meters.
    ...
    along = np.where(along < 0.0, along + 2.0 * np.pi, along)
...
def ecef2enu(
    ...
    """
    Given an origin (lat0,lon0,alt0) convert ECEF (X,Y,Z) to a local ENU
    frame in meters.
    """
    x0, y0, z0 = geod2ecef(lat0, lon0, alt0, ellps=ellps)
    xyz = np.array([x - x0, y - y0, z - z0])
    sinlon0 = np.sin(lon0 * np.pi / 180.0)
    coslon0 = np.cos(lon0 * np.pi / 180.0)
...
def ecef2enu_vel(
    x: ArrayLike, y: ArrayLike, z: ArrayLike, lat0: ArrayLike, lon0: ArrayLike
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Given an origin (lat0,lon0) convert a small offset or velocity in ECEF
    (X,Y,Z) to a local ENU frame.
    """
    assert (
        np.size(x) == np.size(y)
        and np.size(x) == np.size(z)
        and np.size(x) == np.size(lat0)
        and np.size(x) == np.size(lon0)
    )
...
def enu2ecef_vel(
    evel: ArrayLike, nvel: ArrayLike, uvel: ArrayLike, lat0: ArrayLike, lon0: ArrayLike
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def enu2ecef_sigma(
    ...
    """
    Given an origin (lat0,lon0,alt0) convert local ENU covariance matrix to
    ECEF (X,Y,Z).
    ...
    for i in range(np.size(esigma)):
        covi = np.array(
            [
                [esigma[i] * esigma[i], esigma[i] * nsigma[i] * rhoen[i], 0.0],
                [esigma[i] * nsigma[i] * rhoen[i], nsigma[i] * nsigma[i], 0.0],
                [0.0, 0.0, usigma[i] * usigma[i]],
            ]
        )
...
def __Rmat_for_enu2ecef(lat: ArrayLike, lon: ArrayLike) -> np.ndarray:
    ...
    for i in range(np.size(latr)):
        Ri = np.array(
            [
                [-sinlon[i], -sinlat[i] * coslon[i], coslat[i] * coslon[i]],
                [coslon[i], -sinlat[i] * sinlon[i], coslat[i] * sinlon[i]],
                [0.0, coslat[i], sinlat[i]],
            ]
        )
        R_list.append(Ri)
...
def geod2spher(lat: ArrayLike, ellps: Ellipsoid = WGS84) -> Union[float, np.ndarray]:
    ...
    return np.arctan((1 - ellps.e2) * np.tan(np.radians(lat))) * 180.0 / np.pi


def spher2geod(lat: ArrayLike, ellps: Ellipsoid = WGS84) -> Union[float, np.ndarray]:
    """Convert spherical latitude to geodetic."""
    return np.arctan(np.tan(np.radians(lat)) / (1 - ellps.e2)) * 180.0 / np.pi
...
def geod2enu(
    ...
    """
    Given an origin (lat0,lon0,alt0) convert coordinates (lat,lon,alt) to a
    local ENU frame in meters.
...
def translate_flat(
    ...
    """
    # note, the 3rd component of this transformation, the elevation deviation
...
def vincenty(
    ...
    """
    Returns dist, az0, az1 (forward and back azimuths at the starting and
    ending points).
    ...
    while count < 200:
        ...
        sinsig = np.sqrt(
            (cosU1 * np.sin(lam)) ** 2
            + (cosU0 * sinU1 - sinU0 * cosU1 * np.cos(lam)) ** 2
        )
        ...
        if cossqa == 0.0:
            ...
            cos2sigm = cossig - 2.0 * sinU0 * sinU1 / cossqa

        C = (ellps.f / 16.0) * cossqa * (4.0 + ellps.f * (4 - 3.0 * cossqa))
        lam = L + (1.0 - C) * ellps.f * sina * (
            sig + C * sinsig * (cos2sigm + C * cossig * (-1.0 + 2.0 * cos2sigm**2))
        )
        if abs(lam - lam0) < 1.0e-15:
            converged = True
            break
    ...
    if not converged:
        ...
        warnings.warn(
            "Vincenty formula failed to converge (points are nearly antipodal).",
            RuntimeWarning,
        )
    ...
    if sinsig == 0.0:
        ...
        az0 = np.degrees(
            np.arctan2(cosU1 * np.sin(lam), cosU0 * sinU1 - sinU0 * cosU1 * np.cos(lam))
        )
        az1 = 180.0 + np.degrees(
            np.arctan2(
                cosU0 * np.sin(lam), -1 * sinU0 * cosU1 + cosU0 * sinU1 * np.cos(lam)
            )
        )
    return dist, az0, az1
...
def haversine(
    lat0: float, lon0: float, lat1: float, lon1: float, radius: float = 6371000.0
) -> float:
    """
    ...
    lat0r = np.radians(lat0)
    lat1r = np.radians(lat1)
    x = np.sin(dlat / 2.0) * np.sin(dlat / 2.0) + np.sin(dlon / 2.0) * np.sin(
        dlon / 2.0
    ) * np.cos(lat0r) * np.cos(lat1r)
    c = 2.0 * np.arctan2(np.sqrt(x), np.sqrt(1.0 - x))
    dist = radius * c
    return dist
...
def heading(lat0: float, lon0: float, lat1: float, lon1: float) -> float:
    """
    Great-circle heading/azimuth between two points in lat,lon, relative to
    point0 (degrees assumed).
    ...
    headr = np.mod(
        np.arctan2(
            np.sin(lon1r - lon0r) * np.cos(lat1r),
            np.cos(lat0r) * np.sin(lat1r)
            - np.sin(lat0r) * np.cos(lat1r) * np.cos(lon1r - lon0r),
        ),
        2.0 * np.pi,
    )
...
def midpoint(lat0: float, lon0: float, lat1: float, lon1: float) -> Tuple[float, float]:
    ...
    latcr = np.arctan2(
        np.sin(lat0r) + np.sin(lat1r), np.sqrt((np.cos(lat0r) + bx) ** 2 + by**2)
    )
