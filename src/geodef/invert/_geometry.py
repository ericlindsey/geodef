"""Gradient-based nonlinear geometry search (variable projection, JAX).

Private submodule of :mod:`geodef.invert`. The outer search optimizes the
planar geometry vector theta while the inner slip problem is solved
linearly; the jitted variable-projection kernel is cached per
static-shape signature in ``_vp_jitted``.
"""
...
from geodef.invert._results import GeometrySearchResult
