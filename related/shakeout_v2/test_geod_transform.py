...
@pytest.mark.parametrize("lat,lon,alt", [
    (35.0, -106.0, 1500.0),       # Typical mid-latitude
    (0.0, 0.0, 0.0),              # Equator, Prime Meridian
    (90.0, 0.0, 0.0),             # North Pole
    (-90.0, 180.0, 10000.0),      # South Pole, Dateline, High Alt
    (-30.5, 179.99, -500.0),      # Southern hemisphere, Dateline edge, Negative Alt
])
def test_geod2ecef2geod_scalar(lat, lon, alt):
    """Test round trip: geod -> ecef -> geod using scalar inputs."""
    x, y, z = geod_transform.geod2ecef(lat, lon, alt)
    lat_out, lon_out, alt_out = geod_transform.ecef2geod(x, y, z)
...
@pytest.mark.parametrize("lat,lon,alt,lat0,lon0,alt0", [
    (35.1, -106.1, 1500.0, 35.0, -106.0, 1500.0),   # Close points
    (0.0, 90.0, 0.0, 0.0, 0.0, 0.0),                # Far points (Quarter circle)
])
def test_geod2enu2geod_scalar(lat, lon, alt, lat0, lon0, alt0):
    ...
    lat_out, lon_out, alt_out = geod_transform.enu2geod(e, n, u, lat0, lon0, alt0)
...
def test_ecef2enu2ecef_scalar(e, n, u, lat0, lon0, alt0):
    ...
    x_ecef, y_ecef, z_ecef = geod_transform.enu2ecef(e, n, u, lat0, lon0, alt0)
    e_out, n_out, u_out = geod_transform.ecef2enu(x_ecef, y_ecef, z_ecef, lat0, lon0, alt0)
...
def test_geod2ecef_reference_equator():
    ...
    x, y, z = geod_transform.geod2ecef(0.0, 0.0, 0.0)
...
def test_geod2ecef_reference_pole():
    ...
    x, y, z = geod_transform.geod2ecef(90.0, 0.0, 0.0)
...
def test_spher2geod_vs_geod2spher():
    ...
    lat_spher = geod_transform.geod2spher(lat_geod)
    
    # Equator and poles should be identical
    np.testing.assert_allclose(lat_spher[[0, 2, 4]], lat_geod[[0, 2, 4]], atol=ATOL)
    
    # Check if they round trip
    lat_geod_out = geod_transform.spher2geod(lat_spher)
...
def test_coincident_points_distance():
    ...
    lat, lon = 35.0, -106.0
    dist_vincenty, az0, az1 = geod_transform.vincenty(lat, lon, lat, lon)
    dist_haver = geod_transform.haversine(lat, lon, lat, lon)
...
def test_antipodal_distance():
    """Tests for points 180 degrees apart (e.g., pole-to-pole)."""
    # Pole to pole (distance should be half the polar circumference)
    # Using simple apx for flat Earth circumference = 2 * pi * b
    dist, az0, az1 = geod_transform.vincenty(90.0, 0.0, -90.0, 0.0)
    
    # The meridional perimeter (passing through poles) of an ellipsoid with semi-axes a, b
    # approximation is around 20,003 km half-perimeter
    # We will just verify it doesn't crash or return NaN and is roughly 20,000 km
    ...
    with pytest.warns(RuntimeWarning, match="Vincenty formula failed to converge"):
        dist_eq, az0_eq, az1_eq = geod_transform.vincenty(0.0, 0.0, 0.0, 180.0)
    
    assert np.isnan(dist_eq)
    assert np.isnan(az0_eq)
    assert np.isnan(az1_eq)

    # However Haversine should still compute the large spherical distance fine
    dist_haver = geod_transform.haversine(0.0, 0.0, 0.0, 180.0)
    np.testing.assert_allclose(dist_haver, np.pi * 6371000.0, rtol=1e-3)


# -----------------------------------------------------------------------------
# 4. Data Types and Vectorization (numpy ndarray)
# -----------------------------------------------------------------------------
...
def test_ecef2enu_vel_vector():
    ...
    # Velocity components (trivial velocities for testing)
    xvel = np.array([1.0, 0.0])
    yvel = np.array([0.0, 1.0])
    zvel = np.array([0.0, 0.0])

    evel, nvel, uvel = geod_transform.ecef2enu_vel(xvel, yvel, zvel, lat0, lon0)
...
def test_enu2ecef_vel_vector():
    ...
    xvel, yvel, zvel = geod_transform.enu2ecef_vel(evel, nvel, uvel, lat0, lon0)
...
def test_enu2ecef_sigma_vector():
    ...
    rhoen = np.array([0.0, 0.5])  # correlation
    lat0 = np.array([35.0, 45.0])
    lon0 = np.array([-106.0, 120.0])

    cov_out = geod_transform.enu2ecef_sigma(esigma, nsigma, usigma, rhoen, lat0, lon0)
    
    # 2 points = 6x6 output block diagonal matrix
    assert cov_out.shape == (6, 6)


# -----------------------------------------------------------------------------
# 5. Extension Features (Ellipsoids, Pyproj)
# -----------------------------------------------------------------------------
def test_custom_ellipsoid():
    """Test custom ellipsoid inputs."""
    # Define a sphere with radius 6371000m
    sphere = geod_transform.Ellipsoid(a=6371000.0, f=0.0)
    
    # At equator on a sphere, Z should be 0, X should be 6371000
    x, y, z = geod_transform.geod2ecef(0.0, 0.0, 0.0, ellps=sphere)
    np.testing.assert_allclose(x, 6371000.0, atol=ATOL)
    np.testing.assert_allclose(y, 0.0, atol=ATOL)
    np.testing.assert_allclose(z, 0.0, atol=ATOL)
    
    # At poles, Z should be 6371000, X/Y = 0
    x, y, z = geod_transform.geod2ecef(90.0, 0.0, 0.0, ellps=sphere)
    np.testing.assert_allclose(x, 0.0, atol=ATOL)
    np.testing.assert_allclose(y, 0.0, atol=ATOL)
...
def test_pyproj_integration():
    ...
    # Pure Python WGS84
    x1, y1, z1 = geod_transform.geod2ecef(lat, lon, alt)
    
    # EPSG:4326 (WGS84 lat/lon) to ECEF via pyproj
    x2, y2, z2 = geod_transform.geod2ecef(lat, lon, alt, crs="EPSG:4326")
    ...
    # Test inverse
    lat_out, lon_out, alt_out = geod_transform.ecef2geod(x2, y2, z2, crs="EPSG:4326")
    np.testing.assert_allclose(lat_out, lat, atol=ATOL)
    np.testing.assert_allclose(lon_out, lon, atol=ATOL)
    np.testing.assert_allclose(alt_out, alt, atol=ATOL)
