...
Modified Sep 18 2019 - added convert_velocity function
...
def convert_velocity(lat,lon,ve,vn,latp,lonp,degmyr):
    '''4*list + 3*scalar > 2*list
    Returns predicted horizontal (e,n) velocities (in mm/yr) at each location,
    after conversion by the specified pole (last 3 arguments, scalar).
    Inputs are a list of coordinates (lat,lon in degrees) and velocities (e,n)
    and one euler pole (lat, lon, rate(in deg/myr) ).    
    Geodetic (ellipsoid) coordinates in degrees are assumed but the rotation is
    done assuming a spherical earth.''' 
    pole_ve,pole_vn = pole_velocity(lat,lon,latp,lonp,degmyr)
    ve_out = ve + pole_ve
    vn_out = vn + pole_vn
    return ve_out, vn_out
...
def get_2d_covar_mat(esigma, nsigma, rhoen):
    ...
    esigma = np.atleast_1d(esigma)
    nsigma = np.atleast_1d(nsigma)
    rhoen  = np.atleast_1d(rhoen)
    blocks = [
        np.array([[e*e,        e*n*r],
                  [e*n*r,      n*n   ]], dtype=float)
        for e, n, r in zip(esigma, nsigma, rhoen)
    ]
